...
import org.example.soccerlegends.CustomListView;
...
public class Detail extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState){
		...
		for (int j=0;j<CustomListView.list.size();j++)
		{
			if(CustomListView.list.get(j).toString().equalsIgnoreCase(string)) {
				HashMap<String,String> map = new HashMap<String,String>();
				map = CustomListView.list.get(j);
